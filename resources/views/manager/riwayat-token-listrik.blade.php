...
@section('title', 'Riwayat Token Listrik - Ethikopia Stock Management')
        ...
        <div class="col-12">
            <div class="card border-0 shadow-sm">
                <div class="card-body d-flex flex-wrap justify-content-between align-items-center gap-2">
                    ...
                    <div>
                        <a href="{{ route('manager.export.token-listrik') }}" class="btn btn-outline-light btn-sm">
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-body">
                    <div class="table-responsive">
                        <form id="bulkDeleteForm" method="POST" action="{{ route('manager.token-listrik.bulk-delete') }}">
                            @csrf
                            <table class="table table-hover align-middle mb-0">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Shift</th>
                                        <th>Barista</th>
                                        <th style="width: 17%;">R17 (kWh)</th>
                                        <th style="width: 17%;">R18 (kWh)</th>
                                        <th style="width: 17%;">Mesin (kWh)</th>
                                ...
                                <tbody>
                                    @forelse ($records as $i => $rec)
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $rec['shift'] }}</td>
                                            <td>{{ $rec['barista'] }}</td>
                                            <td>{{ format_kwh($rec['token_r17']) }}</td>
                                            <td>{{ format_kwh($rec['token_r18']) }}</td>
                                            <td>{{ format_kwh($rec['token_mesin']) }}</td>
                                            <td>
                                                <div class="d-flex gap-1">
                                                    <a href="{{ route('manager.token-listrik.detail', $rec['id']) }}" class="btn btn-sm btn-outline-info">
...
function openBulkDeleteModal() {
    ...
    swalConfirm({
        ...
        showLoader: true,
        preConfirm: function () {
            return fetch("{{ route('manager.token-listrik.bulk-delete') }}", {
                method: 'POST',
                headers: {
                    'X-Requested-With': 'XMLHttpRequest',
                    'Accept': 'application/json'
